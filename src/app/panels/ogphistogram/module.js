...
function (angular, app, $, _, kbn, moment, timeSeries) {
  ...
  var module = angular.module('kibana.panels.ogphistogram', []);
  app.useModule(module);

  module.controller('ogphistogram', function($scope, $q, querySrv, dashboard, filterSrv) {
    $scope.panelMeta = {
      modals : [
        {
          description: "Inspect",
    ...
    var _d = {
      ...
      resolution  : 100,
      interval    : '5m',
      intervals   : ['auto', '1y', '5y', '10y', '25y', '50y', '100y'],
      fill        : 0,
      linewidth   : 3,
    ...
    $scope.get_interval = function () {
        ...
        $scope.panel.interval = interval || '1y';
    ...
    $scope.isPanelDisabled = function(){
      return isDisabled;
    };
     ...
     * render_panel function to create the histogram.
     *
    ...
    $scope.get_data = function(segment, query_id) {
      ...
      _.each($scope.panel.queries.ids, function(id) {
        ...
        if($scope.panel.mode === 'count') {
          var timeField = filterSrv.getTimeField();
          if (_.isUndefined(timeField)){
            $scope.disablePanel();
            return;
          } else {
            facet = facet.field(timeField);
            $scope.enablePanel();
          }
        } else {
    ...
    $scope.zoom = function(factor) {
      ...
      var arrIds = filterSrv.idsByType('time');


      filterSrv.set({
        type:'time',
        from:moment.utc(_from).toDate(),
        fromDateObj: moment.utc(_from).toDate(),
        to:moment.utc(_to).toDate(),
        toDateObj: moment.utc(_to).toDate(),
        field:time_field
      }, arrIds[0]);
      ...
      link: function(scope, elem) {
        ...
        elem.bind("plothover", function (event, pos, item) {
          ...
          if (item) {
            ...
            $tooltip
              .html(
                group + dashboard.numberWithCommas(value) + " @ " + (scope.panel.timezone === 'utc'? moment.utc(item.datapoint[0]).format('YYYY') : moment(item.datapoint[0]).format('YYYY'))
        ...
        elem.bind("plotselected", function (event, ranges) {
          ...
          filterSrv.set({
            ...
            fromDateObj: moment.utc(ranges.xaxis.from).toDate(),
            to    : moment.utc(ranges.xaxis.to).toDate(),
            toDateObj: moment.utc(ranges.xaxis.to).toDate(),
