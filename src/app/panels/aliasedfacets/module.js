...
define([
  ...
  function(angular, app, _, kbn) {
    ...
    var module = angular.module('kibana.panels.aliasedfacets',[]);
    app.useModule(module);

    module.controller('aliasedfacets', function($rootScope, $scope, fields, querySrv, dashboard, filterSrv) {
      $scope.panelMeta = {
        modals: [{
          description: "Inspect",
          icon: "icon-info-sign",
      ...
      $scope.add_facet_field = function(field, alias) {
      ...
      $scope.add_alias = function (field, alias) {
        ...
        //if (!hasEmptyFields($scope.panel.aliases)) {
      ...
      $scope.get_data = function(segment, query_id) {
        ...
        for (var i = 0; i < $scope.panel.fields.length; i++) {
          facet_fields += '&facet.field=' + getFacetClause($scope.panel.fields[i]);
        }

        // Set the panel's query
        $scope.panel.queries.basic_query = querySrv.getORquery() + fq + facet + facet_fields + rows;
        ...
        results.then(function(results) {
          ...
          if ($scope.query_id === query_id) {
            ...
            var facet_data = {};
            _.each(facet_results, function(counts, field) {
              facet_data[field] = [];
      ...
      $scope.getField = function(alias){
        var field = _.findWhere($scope.panel.aliases, {alias: alias});
      ...
      var getFacetClause = function(field){
        var fc = field;
        var alias = _.findWhere($scope.panel.aliases, {field: field});
        if (!_.isUndefined(alias)){
          fc = "{!key='" + alias.alias + "'}" + fc;
        }
        return fc;
      };
