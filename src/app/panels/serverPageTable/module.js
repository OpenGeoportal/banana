...
define([
    ...
    function (angular, app, _, kbn, moment) {
        ...
        module.controller('serverPageTable', function ($rootScope, $scope, fields, querySrv, dashboard, filterSrv, $filter) {
            $scope.panelMeta = {
                modals: [
                    {
                        description: "Inspect",
            ...
            $scope.toggle_field = function (field, alias) {
                if (_.indexOf($scope.panel.fields, field) > -1) {
                    ...
                    $scope.remove_alias(field);
                } else {
                    $scope.panel.fields.push(field);
                    $scope.add_alias(field, alias);
                }
            };

            $scope.getAlias = function(field){
                var alias = _.findWhere($scope.panel.aliases, {field: field});
            ...
            $scope.add_alias = function (field, alias) {
                ...
                var found = _.findWhere($scope.panel.aliases, {field: field});
                if (_.isUndefined(found)){
                    $scope.panel.aliases.push(qf);
                } else {
                    found.alias = alias;
                }
            };

            $scope.remove_alias = function(field) {
                var alias = _.findWhere($scope.panel.aliases, {field: field});
                if (!_.isUndefined(alias)){
                    delete $scope.panel.aliases[field];
                }
            };
            ...
            $scope.get_field_value = function(field, value){
                var field_type = fields.typeList[field].type;

                var text = "";

                if (field_type === "tdate"){
                    //display date types as YYYY
                    text =  moment(value).format($scope.panel.dateFormat);

                } else {
                    // If image field, then do not truncate, otherwise we will get invalid URIs.
                    var imageFields = $scope.panel.imageFields;
                    if (typeof field !== 'undefined' && imageFields.length > 0 && _.contains(imageFields, field)) {
                        text = $filter('tableDisplayImageField')(text, $scope.panel.imageFieldWidth, $scope.panel.imageFieldHeight);
                    } else {
                        text = $filter('tableHighlight')(value);
                        text = $filter('tableTruncate')(text, $scope.panel.trimFactor, $scope.panel.fields.length);
                    }
                }
